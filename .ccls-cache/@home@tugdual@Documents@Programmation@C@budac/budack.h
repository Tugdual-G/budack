...
void border_iterate(unsigned int depth, long int Npts, double *M_brdr,
                    unsigned int start, float sigma);
void border_start(unsigned int depth, long int Npts, double *M_brdr);
...
void border(unsigned int depth, long int Npts, double *M_brdr, unsigned char *M,
  ...
  /* FILE *fp; */
  float sigma = 2 * dx;
  unsigned int i, nskim = 5;
  float lambda = 2;
  depth = depth / pow(lambda, nskim);
  printf("de = %u \n", depth);

  /* fp = fopen("border.double", "rb"); */
  /* if (fp == NULL) { */
  /*   printf("No file for starting points found, now generating one"); */
  /*   fp = fopen("border.double", "wb"); */
  /*   if (fp == NULL) { */
  /*     printf("error, cannot create starting points file"); */
  /*   } else { */
  /*   } */
  /* } */

  border_start((unsigned int)depth, (unsigned int)Npts, M_brdr);
  printf("first pass \n");
  for (i = 0; i < nskim - 1; i++) {
    depth *= lambda;
    border_iterate(depth, (unsigned int)Npts, M_brdr, (unsigned int)Npts,
                   sigma);
    printf("de = %u \n", depth);
  }
  depth *= lambda;
  border_iterate(depth, Npts, M_brdr, (unsigned int)Npts, sigma);
  printf("de = %u \n", depth);
}
...
void fill(unsigned char *M, double *coordinates, long int N, unsigned int nx,
          unsigned int ny, double dx, float a0, float b0) {
  unsigned int i, j, k = 0;
  for (k = 0; k < N; k += 2) {
    i = (*(coordinates + k) - b0) / dx;
    j = (*(coordinates + k + 1) - a0) / dx;
    if (0 <= i && i < ny && 0 <= j && j < nx) {
      *(M + i * nx + j) += 1;
    } else {
      printf("out of range \n");
    }
